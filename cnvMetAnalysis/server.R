...
shinyServer(function(input, output) {
    ...
    output$cnvAUC <- renderTable({
        ...
        # else {
            ...
            modelChoice <- input$gbmOrglm
            if (modelChoice=="Both"){
                tmp <- combAUC[combAUC$cnv==cnvInput,]
                cnvAUCValue <- round(rowMeans(tmp[,3:7]),5)
                tmp$`mean AUC` <- cnvAUCValue
                tmp1 <- tmp %>% select(cnv,loc,model,`mean AUC`,p_value)

        
            } else {
                modelChoice1 <- gsub("[\\(\\)]", "", regmatches(modelChoice, gregexpr("\\(.*?\\)", modelChoice)))
                tmp <- combAUC[combAUC$cnv==cnvInput & combAUC$model==modelChoice1,]
                cnvAUCValue <- round(rowMeans(tmp[,3:7]),5)
                tmp$`mean AUC` <- cnvAUCValue
                tmp1 <- tmp %>% select(cnv,loc,`mean AUC`,p_value)
            }
    ...
    output$AUCPlot <- renderPlot({
        ...
        tmp1 <- tmp %>% pivot_longer(cols=c(3:7),names_to="rep",values_to="AUC")
        rm(tmp)
        ggplot(tmp1,aes(loc,AUC,colour=model))+geom_boxplot()+theme_bw()+
