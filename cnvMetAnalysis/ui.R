...
dashboardPage(
    ...
    dashboardSidebar(
        ...
        conditionalPanel(
            ...
            # ),
            radioButtons(
                inputId="gbmOrglm",label="Select the classification model to display:",
                choices=c("Gradient Boosting Machine (GBM)",
                          "Generalized Linear Model (GLM)","Both"), 
                selected="Gradient Boosting Machine (GBM)", width="90%"),
        )
    ),
    ...
    dashboardBody(
        ...
        conditionalPanel(
            condition = "input.metOrCNV=='CNV'",
            h5("Performance of COAD tumor tissue classification models 
            ...
            h6("clean: original CNV segment"),
            h6("full: CNV segment+2 up/downstream regions"),
